...
__extend__(DOMText.prototype,{
    ...
    get xml(){
        return __escapeXML__(""+ this.data);
        //return ""+ this.nodeValue;
    },
    toString: function(){
...
var DOMStyleAttr = function(ownerDocument){
    this.DOMNode=DOMNode;
    this.DOMNode(ownerDocument);
    
    this.ownerElement = null;
}
DOMStyleAttr.prototype=new DOMAttr;
__extend__(DOMStyleAttr.prototype, {
    get xml(){
        if(this.nodeValue)
            return this.nodeName+'="'+this.nodeValue.cssText()+'" ';
        else
            return '';
    }
});
...
__extend__(DOMElement.prototype, {	
    ...
    get xml() {
        ...
        ret += "<" + this.tagName.toLowerCase() + ns + attrs +">";
...
XMLP.prototype._parse = function() {
    ...
    var nextChar=this.m_xml.charAt(this.m_iP+1);
    ...
    if(this.m_iP == this.m_xml.indexOf("<", this.m_iP)){
        if(nextChar == "?") {
            ...
            if(this.m_xml.charAt(this.m_iP+2).toUpperCase() == "D") {
        ...
        else if(!this.hasToBeEndOfTag || (nextChar=="/" && this.hasToBeEndOfTag)){
            var iB = this.m_iP;
            var iType = this._parseElement(this.m_iP + 1);
            if(iType==XMLP._ELM_E && this.expectedLowerTagName!=this.m_name){
            
                this.m_iP=iB;
                this._setContent(XMLP._CONT_XML, this.m_iP, this.m_iP++);
        ...
        //dont care in case of &&
        if(nextChar!='&') {
            return this._parseEntity(this.m_iP + 1);
        }
        else {
            this._setContent(XMLP._CONT_XML, this.m_iP, this.m_iP+2);
            this.m_iP+=2;
            return XMLP._TEXT;
        }
    }
...
XMLP.prototype._parseAttribute = function(iB, iE) {
    ...
    if(_SAXStrings.QUOTES.indexOf(cQuote) != -1) {
        ...
        // in case there is no quote around the value
        iVE=iE;
    }
...
var strNU=strN.toUpperCase();

    if(strNU=="META" || strNU=="INPUT" || strNU=="BR"|| strNU=="HR" ){
        iType=XMLP._ELM_EMP;
    }
...
_SAXStrings.prototype.count=function(strD, strF, iB, iE){
    var i=iB;
    var c=0;
    while(i<iE){
        if(strF.indexOf(strD.charAt(i))!=-1){
            c++;
        }
        i++;
    }
...
function __parseLoop__(impl, doc, p, isWindowDocument) {
  ...
  var q = 0;
  var hadToBeEndOfTag = p.hasToBeEndOfTag;
  p.hasToBeEndOfTag = false;
  $debug("Parse loop started");
  while(true) {
    // get next event
    iEvt = p.next();
    if (iEvt == XMLP._ELM_B) {                      // Begin-Element Event
      ...
      if(pName.toLowerCase() == 'script')
        p.hasToBeEndOfTag = true;
    
      p.expectedLowerTagName = pName.toLowerCase();
      
      $debug("found start tag "+pName);
...
//debug()
      ...
      iNodeParent = iNode;                          // descend one level of the DOM Tree
        ...
        $debug("found end tag "+p.getName());
        
        if(p.getName()=="script")
            p.hasToBeEndOfTag=hadToBeEndOfTag;
      ...
      pName = trim(pName, true, true);              // strip spaces from Element name
      $debug("found autoclosing tag"+pName);
  ...
  if (!impl.preserveWhiteSpace) {
    for (var i = 0; i < textNodesList.length; i++) {
        ...
        if (node.parentNode != null) {
            ...
            node.nodeValue = nodeData.replace(/\s/g, ' ');
...
__extend__(DOMDocument.prototype, {	
    ...
    createAttribute : function(name) {
        ...
        if(name=="style")
            node = new DOMStyleAttr(this);
        else        
            node = new DOMAttr(this);
...
__extend__(HTMLElement.prototype, {
		...
		get style(){
		    if(this.$css2props === null){
	            ...
	            DOMElement.prototype.setAttribute.apply(this,["style", this.$css2props]);
	        }
...
var __cssTextToStyles__ = function(css2props, cssText){
    ...
    for ( var i = 0; i < styles.length; i++ ) {
        $info("Adding style property " + styles[i]);
